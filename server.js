...
// 4. CANLI RİSK MOTORU (Gerçek veri ile kapasiteyi çarpıştıran algoritma)
app.get('/api/risk-analizi', async (req, res) => {
  try {
    // A) Gökten GERÇEKTE ne yağıyor?
    const anlikYagis = await getCanliYagis(); 

    // B) Yerdeki boru bunu kaldırır mı?
    const altyapi = getAltyapiVerisi();
